    ...
    Radar.prototype.draw = function() {
      ...
      for(var i = min; i <= max; i += this.settings.step) {
        ...
        if (this.settings.showAxisLabels)
          this.cxt.fillText(Math.round(i*3.3),this.width/2 + this.settings.step * spacing * i+4, this.height/2-2);
      }
	 ...
	 for(var i = 0; i < size; i += 1) {
      ...
      $.each(this.settings.values, function(key,val){
        ...
        var textY = that.height / 2 + Math.sin((Math.PI * 2) * (i / size)) * spacing * val + 20;
        if (textX < that.width/2) {
          textX -= 120
          that.cxt.textAlign="end";
          that.cxt.beginPath();
          var width = that.cxt.measureText(key + 'xxxx').width;
